...
def enemy(args)
    ...
    if args.state.circle_x
        if (args.state.tick_count-args.state.enemy_time)/60 > 3
            if !args.state.e1_x 
            ...
            if args.state.e1_x
                ...
                args.outputs.sprites << [ args.state.e1_x, args.state.e1_y,  args.state.e1_size, args.state.e1_size, 'mygame/sprites/isometric-violet.png ', args.state.e1_direction]  
                collision(args, args.state.e1_x, args.state.circle_x, args.state.e1_y, args.state.circle_y, args.state.e1_size, args.state.sprite_size)
...
def bound(args,x, y, size)
    ...
    args.state.y = 0
    if  y <= 0 
        y = args.state.screen_bound_y - size - 1
    end
    if  y >= args.state.screen_bound_y - size 
        y = 1
    end
    if  x <= 0
        x = args.state.screen_bound_x - size - 1
    end
    if  x >= args.state.screen_bound_x - size 
       x = 1
    end
    args.state.x = x
    args.state.y = y
...
def score(args)
    if args.state.lives <= 1
        if args.state.current_score > args.state.highscore
            args.state.highscore = args.state.current_score
        end
...
def bullet_physics (args)
    ...
    args.state.bullets.each do |e|
        if e.exists
            ...
            args.outputs.sprites << [ e.x, e.y,  e.size, e.size, 'mygame/sprites/circle-black.png', e.direction]
    ...
    until args.state.remove_array_element.size == 0
...
def amimation (args)
    ...
    b = 4
    if time<split*(1/b)
        args.outputs.sprites << [ cordsx, cordsy, 32, 32, 'mygame/app/my_sprites/flaming_sprite_1.png']
        args.outputs.sprites << [ cordsx+offset, cordsy, 32, 32, 'mygame/app/my_sprites/flaming_sprite_1.png']
    elsif (time>=split*(1/b) && time<split*(2/b))||(time>=split*(3/b))
        args.outputs.sprites << [ cordsx, cordsy, 32, 32, 'mygame/app/my_sprites/flaming_sprite_2.png']
        args.outputs.sprites << [ cordsx+offset, cordsy, 32, 32, 'mygame/app/my_sprites/flaming_sprite_2.png']
    elsif time>=split*(2/b) && time<split*(3/b)
        args.outputs.sprites << [ cordsx, cordsy, 32, 32, 'mygame/app/my_sprites/flaming_sprite_3.png']
        args.outputs.sprites << [ cordsx+offset, cordsy, 32, 32, 'mygame/app/my_sprites/flaming_sprite_3.png']
    end
end
...
def collision(args, x1, x2, y1, y2, size1, size2)   
    ...
    if (y1 + size1 >= y2 && x1 + size1 >= x2 && y1 <= y2 + size2 && x1 <= x2 + size2)
        args.state.collisions = 1
    end
end
...
def sprite (args)
    ...
    if args.state.circle_x
        args.outputs.sprites << [ args.state.circle_x, args.state.circle_y,  sprite_size, sprite_size, 'mygame/sprites/shipBlue.png', args.state.circle_direction]

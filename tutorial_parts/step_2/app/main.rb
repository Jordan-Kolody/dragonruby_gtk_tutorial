# This is the tech demo that comes with the gtk.
# The associated tutorial video will briefly go over this content.

# If you dont have access to the audio or the video we will briefly go over it here for ease.
# Remember that what is shown here isn't exclusively what the gtk can do.
# There is so much more that the gtk can do than we can show 
# in a simple demo and intro-tutorial.

# On the top left we have labels, text, which can be modified in a number of ways.
# We can change color, alignment, transparency, size, location and more.

# Bellow that we have lines, solids, and borders.
# Similar to labels these can be modified via color, transparency,
# location start and finish, and more.

# Sprites are next, which are one of the main graphical instruments of a game or program.
# They have many modifications as well, which allow us to specify:
# coordinates, dimensions, colors, and so much more.

# And then the middle consists of inputs and outputs.
# It shows how the different inputs work.
# We have keyboard, mouse, and controller inputs that can taken in and worked with.
# We can track number of presses, the time in which it is pressed,
# the location of the mouse or a sprite when a key or mouse is pressed,
# and we can record if or not a press has happened at all.

# This is not to mention all the other possibilities that are out there.
# There are more ways to use these expressions than is possibly imaginable.
# Its up to the developer, in this case you, to determine what and how they 
# want to set the expressions up in a way that works best for your game or application.

# From this point on we have the tech demo's documentation and explanation.
# Take a look at what you can expect from the gtk, play with some of the values,
# and then move onto the next tutorial section.
# This where we will go through the expressions used to make the tech demo and that will 
# be key in using the gtk.
